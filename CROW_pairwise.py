...
class ClericalApp:
        ...
        if {'Match'}.issubset(working_file.columns):
            ...
            for col_header in working_file.columns:
                ...
                if (col_header == 'Match') or (col_header == 'Comments'):
                    ...
                    # convert to string
                    working_file[col_header] = working_file[col_header].astype(
                        str)
                    # remove nan values
    ...
    def draw_record_frame(self):
        ...
        name_of_datasets = [config['dataset_names'][dset]
                            for dset in config.options('dataset_names')]
        ...
        for dataset_names in config.options('dataset_names'):
            ...
            dataset_names_to_highlight.append(
        ...
        for columnfile_title in config.options('columnfile_info_and_order'):
            ...
            # create a text label
            exec(f'self.{col_header[0]} = Text(self.recordFrame,height=1,relief="flat",bg="gray93")')
                ...
                if col_header[1] == name:
                    ...
                    if col_header[1] in dataset_names_to_highlight:
                        ...
                        if col_header[2] in self.datarows_to_highlight:
                            ...
                            self.datarows_to_highlight[col_header[2]] = [
                                col_header[0]]

                    else:

                        self.datarow_to_compare[col_header[2]] = [
                            col_header[0]]
    ...
    def update_df(self, match_res):
        ...
        if int(config['custom_settings']['commentbox']):
            working_file.at[self.record_index,
...
if __name__ == "__main__":
    ...
    print("\n Number of records matched:", str(
        len(working_file[working_file.Match != ""])))
