...
If you have any questions or suggestions please get in contact using the 
following address: Linkage.Hub@ons.gov.uk

Craig Scott - Original Creator and Developer 
Hannah O'Dair - Developer
...
class ClericalApp:
    ...
    def __init__(self, root, working_file, filename_done, filename_old, config): 
        ...
        for column_title in config.options('column_headers_and_order'):
            ...
            exec(f'self.{column_title}.grid(row=1,column=col_header[1],columnspan=1,padx=10, pady=3)')
        ...
        for columnfile_title in config.options('columnfile_info_and_order'):
            ...
            exec(f'self.{col_header[0]}.config(width=len(working_file["{col_header[0]}"][self.record_index])+2,font=f"Helvetica {self.text_size}",state=DISABLED)')
    ...
    def update_gui(self):
        ...
        for iter_columns in self.iterated_labels:
            ...
            exec(f'self.{iter_columns}.config(width = len(working_file["{iter_columns}"][self.record_index])+2, font=f"Helvetica {self.text_size} {self.text_bold}", state=DISABLED)')
